...
class Dataset:
    def __init__(self, batch_size, folder='vessels', include_hair=True, sgd=False):
        self.folder = folder
        ...
        with tf.device('/gpu:0'):
            ...
            for epoch_i in range(n_epochs):
                ...
                for batch_i in range(dataset.num_batches_in_epoch()):
                    ...
                    # with tf.device('/gpu:0'):
                    cost, cost_orig, _ = sess.run([network.cost, network.cost_orig, network.train_op],
                                       feed_dict={network.inputs: batch_inputs, network.targets: batch_targets,
                                                  network.is_training: True,
                                                  network.ce_weight: pos_weight})
                    end = time.time()
                    print('{}/{}, epoch: {}, cost: {}, cost_orig: {}. batch time: {}, positive_weight: {}'.format(batch_num,
                                                                                                   n_epochs * dataset.num_batches_in_epoch(),
                                                                                                   epoch_i, cost, cost_orig,
                                                                                                   end - start,
                                                                                                   pos_weight))
                    if batch_num % 1000 == 0 or batch_num == n_epochs * dataset.num_batches_in_epoch():
                        ...
                        print(
                            'Step {}, cost: {},  cost_orig: {}. test accuracy: {}, dice_coe {}, hard_dice {}, iou_coe {}, recall {}, precision {}, fbeta_score {}, auc {}, specificity {}'.format(
                                batch_num, cost, cost_orig, test_accuracy, dice_coe_val.eval(), hard_dice_coe_val.eval(),
                                iou_coe_val.eval(),
                        ...
                        f1.write(
                            'Step {}, cost: {}, cost_orig: {}. test accuracy: {}, dice_coe {}, hard_dice {}, iou_coe {}, recall {}, precision {}, fbeta_score {}, auc {}, specificity {}, max acc {} {} \n'.format(
                                batch_num, cost, cost_orig, test_accuracy, dice_coe_val.eval(), hard_dice_coe_val.eval(),
